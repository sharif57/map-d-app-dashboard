...
import { useTrustSafetyQuery, useUpdateTrustSafetyMutation } from "../../redux/features/privacySlice";
...
const EditTrust = () => {
  ...
  const { data, isLoading, isError } = useTrustSafetyQuery();
  const [updateTrustSafety, { isLoading: isUpdating }] = useUpdateTrustSafetyMutation();
  ...
  const handleUpdate = async () => {
    try {
      await updateTrustSafety({ id: data?.data?.id, description: content }).unwrap();
      navigate(-1); // Redirect to a relevant page after update
    } catch (error) {
      console.error("Failed to update trust and safety:", error);
    }
  };

  if (isLoading) return <div>Loading...</div>;
  if (isError) return <div>Error loading trust and safety data.</div>;
        ...
        <div className="space-y-[24px] min-h-[83vh] bg-light-gray rounded-2xl">
          ...
          <div className="w-full px-16">
            <div className="h-full border border-gray-400 rounded-md">
              <ReactQuill
                placeholder="Enter your updated trust & safety content..."
                theme="snow"
                value={content}
