...
import { useAllContinentsQuery, useAllCountryListQuery, useUpdateCountryMutation } from "../../../redux/features/useSlice";
...
export default function Country() {
  ...
  const {data : continents} = useAllContinentsQuery();
  ...
  const countries = data?.data?.map((country, index) => ({
    ...
    description: country.description || "N/A",
  })) || [];
  ...
  //     population: country.population.includes("million")
  ...
  const showModal = (country) => {
  const selectedContinentId = continents?.data?.find(
    (c) => c.name === country.continent
  )?.id;
  ...
  form.setFieldsValue({
    ...
    region: country.region,
    capital: country.capital,
    continent: selectedContinentId, // ✅ ID
    language: country.language,
  ...
  //     population: Number(values.population), // Convert back to raw number
  ...
  const payload = {
    ...
    region: values.region,
    capital: values.capital,
    continent: values.continent, // ✅ ID (number)
    official_language: values.language,
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
        ...
        >
          <Form form={form} onFinish={handleUpdate} layout="vertical">
            <Form.Item name="name" label="Name" rules={[{ required: true, message: "Please enter the country name" }]}>
              <Input />
            </Form.Item>
            <Form.Item name="region" label="Region" rules={[{ required: true, message: "Please enter the region" }]}>
              <Input />
            </Form.Item>
            <Form.Item name="capital" label="Capital" rules={[{ required: true, message: "Please enter the capital" }]}>
              <Input />
            </Form.Item>
...
>
  ...
  />
            ...
            <Form.Item
              ...
              rules={[{ required: true, message: "Please enter the official language" }]}
            >
              <Input />
            </Form.Item>
            <Form.Item
              ...
              rules={[{ required: true, message: "Please enter the population" }]}
            >
