...
//             <button
//               type="submit"
//               className={`w-full bg-[#3698dc] hover:bg-[#3698dc] text-white font-medium py-3 px-4 rounded-full transition duration-200 ${isLoading ? "opacity-70 cursor-not-allowed" : ""}`}
//               disabled={isLoading}
//             >
...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(res?.message, "reset response");
    } catch (error) {
      let errorMessage =
        error?.data?.message ||
        error?.errors?.non_field_errors?.[0] ||
      ...
      toast.error(errorMessage || "Reset password failed");
